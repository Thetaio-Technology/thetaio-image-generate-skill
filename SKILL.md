...
- **单张超过 4MB** 的参考图,自动去掉 alpha 通道并转成 **JPEG(q90,保留原分辨率)**,再拼进上传请求体。体积通常能降到 **1/5 ~ 1/10**,画质肉眼几乎无损。
- **单张不超过 4MB** 的参考图原样上传,不做任何处理。
- 实时打印 `[压缩] 文件名 原大小 -> 新大小`,便于确认。
- 这一步只影响**上传的参考图**,不改用户的原始文件,也不影响输出图。
- 目的: 单个请求体拼装会整体驻留网关内存并被放大数倍,**超大参考图在并发时会把网关内存打爆**(实测单个 45MB 请求会占用约 700MB 内存)。压缩后 30 并发也能稳定运行。

- 需要保留原始 PNG(例如后续还要二次迭代)时,给单个生图或批量脚本加 `--no-compress` 关闭。
...
脚本支持两种请求方式,用 `--mode` 控制,默认 `auto`:

- `auto`(默认,推荐): 脚本自己按请求大小判断。**参考图 ≥ 2 张、参考图总体积 ≥ 4MB、`--n` 大于 1、或 `--size 4K` 时走异步;其余走同步。**
- `sync`: 强制同步。一次请求一直等到出图(通常 30~180 秒),期间连接必须保持不断。
- `async`: 强制异步。提交后立刻拿到任务号,脚本自动轮询直到完成,再取图。

两种方式**对用户是透明的**: 无论走哪种,最后都得到同样的图片文件;区别只在底层。

- **同步**: 请求挂在那里等图,长等待期间网络抖动、代理中断都可能让整单失败。适合小请求(单张、小图)。
- **异步**: 提交立即返回,后台慢慢画,脚本轮询取结果。**长等待不再占用连接,更抗抖动,多张参考图或大请求更稳。** 适合参考图多、体积大、4K、多张输出的场景。

`auto` 已经覆盖了这些经验规则,所以**大多数情况直接用默认值即可**,不需要用户手动选。只有用户明确说"用同步"或"用异步"时才传 `--mode sync` 或 `--mode async`。

对话中如需向用户解释,可用一段大白话: 同步像"站在柜台干等出图",异步像"拿个取餐号先走、好了再凭号来取";异步更适合长时间、大请求,不容易因为网络抖动失败。
...
> 参考图上传前会自动做边界判断: 单张 > 4MB 的图会转成 JPEG(q90,原分辨率)再上传, 明显缩小请求体, 避免大图并发把网关内存打爆。需要原样上传时加 `--no-compress`。
...
- **上传参考图前自动做边界判断**: 单张 > 4MB 的参考图自动转 JPEG(q90,原分辨率)再上传。这是并发稳定性要求,默认开启;只有用户明确要求保留原始大图时才用 `--no-compress`。
- 默认不要自动压缩**输出图**。只有用户明确要求压缩、发布包、上传前处理或转格式时,才执行 `scripts/compress_image.py`。
