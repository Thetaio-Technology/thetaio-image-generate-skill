...
- `python`
- Python 包: `Pillow`(压缩脚本使用;同时用于参考图上传前的自动压缩。未安装时生图仍可用,但超过阈值的参考图会原样上传,不再自动瘦身)
...
> 参考图上传前会自动做边界判断: 单张 > 4MB 转 JPEG(q90,原分辨率)再上传,缩小请求体,避免大图并发把网关内存打爆; 需要原样上传时加 `--no-compress`。
